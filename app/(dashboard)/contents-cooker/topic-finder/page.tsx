...
export default function TopicFinderPage() {
    ...
    // topicResults 상태를 수정 가능하게 관리
    const handleTopicChange = (idx: number, newVal: string) => {
        setTopicResults(prev => prev.map((t, i) => i === idx ? { ...t, topic: newVal } : t));
    };
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#F2F2F2]">
            <div className="w-full max-w-5xl mx-auto bg-white rounded-2xl shadow-xl p-10 flex flex-col items-center">
                {/* 중앙 정렬 인사말 */}
                                    ...
                                    <AlertDialogAction
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                if (!res.ok) throw new Error('API error');
                                                const data = await res.json();
                                                let newTopics = Array.isArray(data) ? data : data.topics;
                                                if (!Array.isArray(newTopics)) {
                                                    toast.error('Invalid topic result format');
                                                    return;
                                                }
                                                setTopicResults(prev => [...prev, ...newTopics.map(topic => ({ topic }))]);
                                            } catch (e) {
